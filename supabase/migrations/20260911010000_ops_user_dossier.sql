...
begin
  ...
  select * into v_user from public.users u where u.uid::text = p_uid;
  if not found then
  ...
  left join public.account_controls c on c.uid::text = p_uid;
  ...
  return jsonb_build_object(
    ...
    'learning', jsonb_build_object(
      'enrollments_by_status', coalesce((select jsonb_object_agg(coalesce(g.status::text, 'unknown'), g.n)
        from (select e.status, count(*) as n from public.enrollments e
          where e.user_id::text = p_uid group by e.status) g), '{}'::jsonb),
      'recent_enrollments', coalesce((select jsonb_agg(r order by r.created_at desc nulls last)
        from (select e.id, e.course_id, e.course_title, e.status, e.source, e.progress_percent, e.created_at
          from public.enrollments e where e.user_id::text = p_uid
          order by e.created_at desc nulls last limit 10) r), '[]'::jsonb),
      'certificates', (select count(*) from public.certificates ce where ce.user_id::text = p_uid),
      'course_subscriptions_by_status', coalesce((select jsonb_object_agg(coalesce(g.status::text, 'unknown'), g.n)
        from (select cs.status, count(*) as n from public.course_subscriptions cs
          where cs.user_id::text = p_uid group by cs.status) g), '{}'::jsonb)),
    ...
    'behavior', jsonb_build_object(
      'last_activity_at', greatest(
        ...
        (select max(lp.completed_at::timestamptz) from public.lesson_progress lp where lp.user_id::text = p_uid),
        (select max(pb.last_seen_at::timestamptz) from public.lesson_playback pb where pb.user_id::text = p_uid),
        (select max(p.created_at::timestamptz) from public.community_posts p where p.author_id::text = p_uid),
        (select max(cc.created_at::timestamptz) from public.community_comments cc where cc.author_id::text = p_uid),
        (select max(lc.created_at::timestamptz) from public.lesson_comments lc where lc.author_id::text = p_uid),
        (select max(m.created_at::timestamptz) from public.course_messages m where m.sender_id::text = p_uid)),
      'lessons_completed', (select count(*) from public.lesson_progress lp
        where lp.user_id::text = p_uid and lp.completed_at is not null),
      'lessons_opened', (select count(*) from public.lesson_playback pb where pb.user_id::text = p_uid),
      'messages_sent', (select count(*) from public.course_messages m where m.sender_id::text = p_uid),
      'wishlist', (select count(*) from public.wishlists w where w.user_id::text = p_uid),
      'points', (select ms.points from public.member_stats ms where ms.uid::text = p_uid),
      'level', (select ms.level from public.member_stats ms where ms.uid::text = p_uid),
      'last_30_days', jsonb_build_object(
        'lessons_completed', (select count(*) from public.lesson_progress lp
          where lp.user_id::text = p_uid and lp.completed_at::timestamptz >= v_since),
        'posts', (select count(*) from public.community_posts p
          where p.author_id::text = p_uid and p.created_at::timestamptz >= v_since),
        'comments', (select count(*) from public.community_comments cc
          where cc.author_id::text = p_uid and cc.created_at::timestamptz >= v_since),
        'lesson_comments', (select count(*) from public.lesson_comments lc
          where lc.author_id::text = p_uid and lc.created_at::timestamptz >= v_since),
        'messages', (select count(*) from public.course_messages m
          where m.sender_id::text = p_uid and m.created_at::timestamptz >= v_since)),
      'advisor', jsonb_build_object(
        'conversations', (select count(*) from public.advisor_conversations ac where ac.teacher_id::text = p_uid),
        'messages', (select count(*) from public.advisor_messages am
          join public.advisor_conversations ac on ac.id = am.conversation_id where ac.teacher_id::text = p_uid),
        'last_used_at', (select max(ac.updated_at::timestamptz)
          from public.advisor_conversations ac where ac.teacher_id::text = p_uid))),
    ...
    'timeline', coalesce((select jsonb_agg(ev order by ev.at desc) from (
        select u.kind, u.at, u.label from (
          (select 'course_created'::text, co.created_at::timestamptz, co.title::text
            from public.courses co where co.owner_id::text = p_uid
    ...
    'purchases', jsonb_build_object(
      'orders', coalesce((select jsonb_agg(g order by g.status, g.currency)
        from (select o.status, upper(o.currency::text) as currency, count(*) as n,
            coalesce(sum(o.amount_minor), 0) as amount_minor,
            coalesce(sum(o.refunded_amount_minor), 0) as refunded_minor
          from public.orders o where o.user_id::text = p_uid
          group by o.status, upper(o.currency::text)) g), '[]'::jsonb),
      ...
      'plan_subscription', (select to_jsonb(s) from (
          ...
          from public.subscriptions sb where sb.user_id::text = p_uid
    ...
    'creator', jsonb_build_object(
      ...
      'verification_case', (select jsonb_build_object(
        ...
        from public.creator_verification_cases v where v.creator_id::text = p_uid
      ...
      'activation_waiver', (select jsonb_build_object('granted_at', w.granted_at, 'ready_at', w.ready_at)
        from public.creator_activation_waivers w where w.uid::text = p_uid),
      ...
      'courses_by_status', coalesce((select jsonb_object_agg(coalesce(g.status::text, 'unknown'), g.n)
        from (select co.status, count(*) as n from public.courses co
          where co.owner_id::text = p_uid group by co.status) g), '{}'::jsonb),
      ...
      'sales', coalesce((select jsonb_agg(g order by g.status, g.currency)
        from (select o.status, upper(o.currency::text) as currency, count(*) as n,
            coalesce(sum(o.amount_minor), 0) as amount_minor,
            coalesce(sum(o.refunded_amount_minor), 0) as refunded_minor
          from public.orders o where o.teacher_id::text = p_uid
          group by o.status, upper(o.currency::text)) g), '[]'::jsonb)),
    'community', jsonb_build_object(
      'posts', (select count(*) from public.community_posts p where p.author_id::text = p_uid),
      'comments', (select count(*) from public.community_comments cc where cc.author_id::text = p_uid),
      'lesson_comments', (select count(*) from public.lesson_comments lc where lc.author_id::text = p_uid),
      'reports_filed', (select count(*) from public.community_reports cr where cr.reporter_id::text = p_uid),
      'reports_against_by_status', coalesce((select jsonb_object_agg(coalesce(g.status::text, 'unknown'), g.n)
        from (select cr.status, count(*) as n from public.community_reports cr
          where cr.target_author_id::text = p_uid group by cr.status) g), '{}'::jsonb)),
    'support', jsonb_build_object(
      'tickets_by_status', coalesce((select jsonb_object_agg(coalesce(g.status::text, 'unknown'), g.n)
        from (select t.status, count(*) as n from public.support_tickets t
          where t.user_id::text = p_uid group by t.status) g), '{}'::jsonb),
      'recent_tickets', coalesce((select jsonb_agg(r order by r.created_at desc nulls last)
        from (select t.id, t.subject, t.status, t.created_at from public.support_tickets t
          where t.user_id::text = p_uid order by t.created_at desc nulls last limit 5) r), '[]'::jsonb)),
    'privacy_requests', coalesce((select jsonb_agg(r order by r.requested_at desc nulls last)
      from (select q.id, q.type, q.status, q.requested_at, q.resolved_at
        from public.account_action_requests q where q.requested_by::text = p_uid
    ...
    'audit', jsonb_build_object(
      'on_user', coalesce((select jsonb_agg(r order by r.created_at desc nulls last)
        from (select l.id, l.action, l.summary, l.actor_email, l.metadata ->> 'reason' as reason, l.created_at
          from public.audit_log l where l.target_type = 'user' and l.target_id::text = p_uid
      ...
      'by_user', coalesce((select jsonb_agg(r order by r.created_at desc nulls last)
        from (select l.id, l.action, l.summary, l.target_type, l.target_id, l.created_at
          from public.audit_log l where l.actor_id::text = p_uid
          order by l.created_at desc nulls last limit 20) r), '[]'::jsonb)));
end;
$function$;
