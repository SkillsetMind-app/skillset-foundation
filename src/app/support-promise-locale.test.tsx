...
import { generateMetadata as loadingMetadata } from "@/app/loading/page";
...
vi.mock("@/components/auth/loading-screen", () => ({ LoadingScreen: () => null }));
...
describe("support and Promise server pages use the cookie and real dictionaries", () => {
  ...
  it.each(["en", "es"] as const)("localizes the loading page tab title in %s", async locale => {
    mocks.locale = locale;
    expect(await loadingMetadata()).toMatchObject({
      title: locale === "es" ? "Preparando tu espacio de trabajo | SkillsetMind" : "Preparing your workspace | SkillsetMind",
