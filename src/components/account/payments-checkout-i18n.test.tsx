...
vi.mock("@stripe/react-stripe-js", () => ({
  ...
  EmbeddedCheckout: () => <input aria-label="Card fixture" />,
}));
...
describe("payment checkout localization with real dictionaries", () => {
  it.each(["activation", "billing"])("keeps %s request, loader, options and entered data on locale changes", async (kind) => {
    ...
    const input = await screen.findByRole("textbox", { name: "Card fixture" });
    fireEvent.change(input, { target: { value: "entered fixture" } });
    ...
    expect(screen.getByRole("textbox", { name: "Card fixture" })).toBe(input);
    expect(input).toHaveValue("entered fixture");
  ...
  it("keeps the activation amount, Free commission and processing estimates", async () => {
    ...
    expect(screen.getByText(formatUsdWhole(activationFeeUsd))).toBeInTheDocument();
  ...
  it.each(plans.filter((plan) => plan.id !== "free").flatMap((plan) =>
    ...
    expect(screen.getByText(copy("es", cycle === "yearly" ? "billingCheckout.billedYearly" : "billingCheckout.billedMonthly")
      .replace("{amount}", () => formatUsdWhole(total)))).toBeInTheDocument();
  ...
  it.each([
    ...
    expect(mocks.fetch).toHaveBeenCalledTimes(1);
    expect(mocks.failed).toHaveBeenCalledWith({ course_id: "plan:starter:monthly", reason: `billing_checkout_${key}` });
  });
  ...
  it("localizes modal and close labels without replacing checkout, and preserves Escape/scroll recovery", async () => {
    ...
    await screen.findByRole("textbox");
    const initial = mocks.provider.mock.calls.at(-1)![0];
    expect(screen.getByRole("dialog", { name: "Confirma tu cambio de plan" })).toBeInTheDocument();
    expect(screen.getAllByRole("button", { name: "Cerrar cambio de plan" })).toHaveLength(2);
    fireEvent.click(screen.getByText("EN"));
