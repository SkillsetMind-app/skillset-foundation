...
vi.mock("@stripe/react-connect-js", () => ({
  ...
  ConnectAccountOnboarding: (props: { onLoadError: (error: LoadError) => void; onExit: () => void }) => {
    ...
    return <div data-testid="connect-onboarding-fixture" />;
  },
}));
function Languages() {
...
it("updates Connect locale in place, keeps entered data, and preserves session renewal and completion", async () => {
  ...
  mount({ onComplete });
  // Same DOM node after the locale change = Stripe's form was not remounted.
...
it.each([
  ...
  expect(screen.getByRole("button", { name: copy("es", "connectOnboarding.continueSecure") })).toBeInTheDocument();
  fireEvent.click(screen.getByText("EN"));
