...
vi.mock("@/components/platform/platform-shell", () => ({
  // Same shape as residual-routes-locale.test.tsx: the real shell does not render
  // this title (hideHeader), so the test reads the prop instead of a named region.
...
vi.mock("@/components/teacher/teacher-community-inbox", () => ({
  TeacherCommunityInbox: ({ courseId }: { courseId: string }) => {
    ...
    return <div data-testid="community-inbox" data-course-id={courseId} />;
  },
}));
...
describe("teacher wrapper titles and Suspense use the real server dictionaries", () => {
  it.each([
    ...
    expect(screen.getByRole("main")).toHaveAttribute("data-title", media);
    expect(screen.getByRole("main")).toHaveAttribute("data-hide-header", "true");
    first.unmount();
    const second = render(await MembersPage());
    expect(screen.getByRole("main")).toHaveAttribute("data-title", members);
    expect(screen.getByRole("main")).toHaveAttribute("data-hide-header", "true");
    second.unmount();
    const params = Promise.resolve({ courseId: "original/$&" });
    const third = render(await CommunityPage({ params }));
    expect(screen.getByRole("main")).toHaveAttribute("data-title", community);
    expect(screen.getByRole("main")).toHaveAttribute("data-hide-header", "true");
    expect(screen.getByTestId("community-inbox")).toHaveAttribute("data-course-id", "original/$&");
    third.unmount();
    state.suspend = true;
