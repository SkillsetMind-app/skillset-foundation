...
vi.mock("@/components/learn/community-feed", () => ({
  // Mirrors community-feed.tsx: the header strips the fixed " community" suffix.
  CommunityFeed: ({ space }: { space: CommunitySpace }) => <div><h2>{space.name.replace(/ community$/i, "")}</h2><p>{space.description}</p></div>,
}));
...
describe("remaining classroom copy uses real dictionaries", () => {
  ...
  it("localizes only generated community metadata, preserving the course title literally", () => {
    ...
    expect(screen.getByRole("heading", { level: 2, name: "Original course $&" })).toBeVisible();
    ...
    fireEvent.click(screen.getByRole("button", { name: "Change language" }));
    expect(screen.getByRole("heading", { level: 2, name: "Original course $&" })).toBeVisible();
    expect(screen.getByText("A course-linked space for announcements, questions, resources, and cohort discussion.")).toBeVisible();
  });
