...
export function buildContentSecurityPolicy(nonce: string): string {
  ...
  // Realtime is a socket, never a <video> source: media-src takes the https
  // origin only, so the wss entry does not leak into a directive that cannot
  // use it.
  const supabaseMedia = supabase[0] ? ` ${supabase[0]}` : "";
  return [
    "default-src 'self'",
    "base-uri 'self'",
    "object-src 'none'",
    ...
    "font-src 'self' data:",
    `connect-src 'self'${supabaseConnect} https://api.stripe.com https://api.pwnedpasswords.com https://us.i.posthog.com https://us-assets.i.posthog.com https://va.vercel-scripts.com https://vitals.vercel-insights.com https://video.bunnycdn.com`,
    ...
    `media-src 'self' blob:${supabaseMedia} https://iframe.mediadelivery.net`,
