...
export function isConnectNotEnabledError(error: unknown): boolean {
  ...
  return /signed up for Connect|connect_not_enabled|finishing its Stripe Connect setup|Only Stripe Connect platforms can work|complete your platform profile to use Connect/i.test(
    message,
  );
}
